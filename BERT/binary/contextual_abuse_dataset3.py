...
CATEGORY_NAMES = ['Neutral', 'Abusive Speech']
        ...
        for id_, row in df.iterrows():
           ...
           #Skip entries that should be ignored
            if ignore_entry(text):
                continue
            # Handle labels
            # Map all non-neutral and non-counterspeech categories to 'Abusive Speech'
            if pd.isna(row['annotation_Primary']) or row['annotation_Primary'] in ['Neutral','Slur', 'CounterSpeech']:
                label = 'Neutral'
            else:
                label = 'Abusive Speech'

            id_to_data[info_id]['labels'].add(label)
